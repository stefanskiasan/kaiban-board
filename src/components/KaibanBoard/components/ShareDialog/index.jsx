...
const ShareDialog = () => {
  ...
  const { isLoadingShare, shareTeamAction, setShareDialogOpenAction } =
      ...
      <div className="kb-flex kb-min-h-full kb-items-center kb-justify-center kb-p-4">
        ...
        >
          ...
          <p className="kb-text-xs kb-text-slate-600">
            Your agents and team will be publicly visible on the internet.
            Ensure all information is suitable for public sharing.
          </p>
            ...
            <div className="kb-grid kb-grid-cols-[auto_1fr] kb-gap-2 kb-mb-2">
              {executed.current && (
                <div className="kb-relative kb-group kb-flex kb-items-center">
                  <AgentAvatar
                    agent={{ name: userName !== '' ? userName : 'Username' }}
                    size="xl"
                  />
                  {userName === defaultUsername.username && (
                    <Tooltip
                      text={defaultUsername.explanation}
                      styles="kb-left-[22px]"
                    />
                  )}
                </div>
          ...
          <div className="kb-mt-6 kb-flex kb-gap-2 kb-items-center">
            {isLoadingShare && (
              <div className="kb-flex kb-items-center kb-gap-2">
                <Spinner />
                <span className="kb-text-xs kb-font-medium kb-text-slate-400">
                  Sharing team details...
                </span>
              </div>
            )}
            <Button
