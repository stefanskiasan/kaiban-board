...
const MissingKeysDialog = () => {
  ...
  const { teamStore, setMissingKeysDialogOpenAction } =
      ...
      <div className="kb-flex kb-min-h-full kb-items-center kb-justify-center kb-p-4">
        ...
        >
          ...
          <p className="kb-mt-2 kb-text-sm/6 kb-text-slate-400">
            Please configure the environment variables in the .env file by
            setting the keys listed below.
          </p>
          <div className="kb-mt-4">
            <div className="kb-flex kb-flex-col kb-gap-2 kb-mb-2">
              {keys.map((key, idx) => (
                <div
                  key={idx}
                  className="kb-flex kb-items-stretch kb-gap-3 kb-relative kb-bg-white/5 kb-p-3 kb-rounded-lg kb-ring-1 kb-ring-slate-900"
                >
                  ...
                  <div key={idx} className="kb-flex kb-flex-col kb-gap-2">
                        ...
                        <div className="kb-relative kb-p-3">
                          ...
                          <div className="kb-absolute kb-top-2 kb-right-2 kb-h-8 kb-flex kb-items-center">
                            <div className="kb-relative kb-flex">
                              <button
                                type="button"
                                className="kb-text-slate-400 hover:kb-text-slate-200"
                                onClick={() => {
                                  navigator.clipboard.writeText(
                                    `VITE_${key.key}=your-api-key-value`
                                  );
                                }}
                              >
                    ...
                    <span
                      className="kb-text-xs kb-font-normal kb-text-slate-400"
                      dangerouslySetInnerHTML={{ __html: key.get }}
                    ></span>
                  </div>
                </div>
              ))}
