...
const ExampleTeams = () => {
    ...
    return (
        <Popover>
            <PopoverButton disabled={teamWorkflowStatus === 'RUNNING' || !exampleTeams.length} className="kb-relative kb-group kb-w-min kb-flex kb-items-center kb-p-2 kb-text-sm kb-font-medium kb-text-slate-400 focus:kb-outline-none data-[hover]:kb-text-indigo-500 data-[focus]:kb-outline-1 data-[focus]:kb-outline-white data-[disabled]:kb-text-slate-600">
            ...
            >
                <div className="kb-p-1">
                    {exampleTeams.map((team , idx) => (
                        <CloseButton key={idx} className="kb-group kb-block kb-w-full kb-rounded-lg kb-py-2 kb-px-3 kb-transition hover:kb-bg-indigo-500/15"
                            onClick={() => { setExampleCodeAction(team) }}>
                            <p className="kb-text-sm kb-font-medium kb-text-slate-400 kb-text-left group-hover:kb-text-indigo-500">{extractTeamName(team().code)}</p>
                        </CloseButton>
                    ))}
                </div>
            </PopoverPanel>
