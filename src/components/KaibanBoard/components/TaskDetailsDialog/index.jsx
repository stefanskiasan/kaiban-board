...
import {
  copyToClipboard,
  filterAndFormatAgentLogs,
  filterAndExtractMetadata,
...
const TaskDetailsDialog = () => {
  ...
  const { teamStore, selectedTask, setTaskDetailsDialogOpenAction } =
  ...
  const { workflowLogs, teamWorkflowStatus, validateTask, provideFeedback } =
    teamStore(state => ({
  ...
  useEffect(() => {
    const unsubscribeFromTeamStore = teamStore.subscribe((state, prev) => {
      if (state.workflowLogs !== prev.workflowLogs) {
        const logs = filterAndFormatAgentLogs(
          state.workflowLogs,
          selectedTask.id
        );
        setLogs(logs);

        const stats = filterAndExtractMetadata(
          state.workflowLogs,
          selectedTask.id
        );
        setStats(stats);

        const awaiting = isAwaitingValidation(
          state.workflowLogs,
          selectedTask.id
        );
  ...
  return (
    <div className="kb-absolute kb-w-full kb-h-full kb-inset-0 kb-bg-slate-950/50 kb-overflow-auto">
      <div className="kb-flex kb-min-h-full kb-items-center kb-justify-center kb-p-4">
        <div
          ref={containerRef}
          className="kb-z-40 kb-w-full kb-max-w-lg kb-rounded-xl kb-bg-white/5 kb-p-6 kb-backdrop-blur-2xl"
        >
          {/* --- HEADER --- */}
          ...
          <div className="kb-mt-8 kb-pl-[36px] kb-flex kb-gap-6">
            <div>
              <span className="kb-text-xs kb-font-medium kb-text-slate-200">
                Members
              </span>
            ...
            <div>
              ...
              <div className="kb-flex kb-items-center kb-mt-1 kb-h-[32px]">
                <ModelLogo
                  model={
                    task?.agent?.agentInstance?.llmConfig?.provider || 'openai'
                  }
                  size={24}
                />
              </div>
            ...
            <div>
              ...
              <div className="kb-flex kb-items-center kb-mt-1 kb-h-[32px]">
                <span className="kb-text-xs kb-font-normal kb-text-slate-400">
                  {task?.agent?.agentInstance?.llmConfig?.model || 'gpt-3.5'}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="kb-flex kb-items-center kb-gap-4 kb-mt-8 kb-text-slate-400">
            ...
            <h3 className="kb-text-base/7 kb-font-medium">Description</h3>
          </div>
          ...
          <div className="kb-flex kb-items-center kb-gap-4 kb-mt-8 kb-text-slate-400">
            ...
            <h3 className="kb-text-base/7 kb-font-medium">Result</h3>

            {task?.result && (
              <Button
                className="kb-ml-auto kb-inline-flex kb-items-center kb-gap-2 kb-rounded-lg kb-bg-slate-900 kb-py-1 kb-px-2.5 kb-text-slate-400 kb-text-sm focus:kb-outline-none data-[hover]:kb-bg-indigo-500/15 data-[hover]:kb-text-indigo-500 data-[open]:kb-bg-gray-700 data-[focus]:kb-outline-1 data-[focus]:kb-outline-white"
                onClick={() => {
                  copyToClipboard(
          ...
          {task?.result ? (
            <div
              className="kb-p-4 kb-mt-2 kb-rounded-lg"
            ...
            <div className="kb-pl-[36px] kb-min-w-full">
              <span className="kb-text-slate-400 kb-text-xs kb-font-normal">
                {'No result in this task'}
              </span>
            </div>
          )}
          {/* --- RESULT --- */}
          ...
          <div className="kb-flex kb-items-center kb-gap-4 kb-mt-8 kb-text-slate-400">
            ...
            <h3 className="kb-text-base/7 kb-font-medium">Stats</h3>
          </div>
          <div className="kb-pl-[36px]">
            {stats ? (
              <div className="kb-flex kb-gap-x-6 kb-gap-y-2 kb-flex-wrap">
                <div className="kb-flex kb-gap-1">
                  ...
                  <span className="kb-text-xs kb-font-normal kb-text-slate-400">
                    {stats.llmUsageStats?.inputTokens +
                      stats.llmUsageStats?.outputTokens}
                  </span>
                </div>
                <div className="kb-flex kb-gap-1">
                  <span className="kb-text-xs kb-font-medium kb-text-slate-200">
          ...
          <div className="kb-flex kb-items-center kb-gap-4 kb-mt-8 kb-text-slate-400">
            ...
            <h3 className="kb-text-base/7 kb-font-medium">Activity</h3>
          </div>
          {logs && logs.length !== 0 ? (
            <>
              {/* ACTIONS */}
                        ...
                        <div className="kb-flex kb-flex-col kb-gap-2">
                          ...
                          <div className="kb-flex kb-gap-2">
                            <Button
                              ...
                              disabled={feedback === ''}
                            >
                              <span className="kb-hidden md:kb-block">
                                Submit Feedback
                              </span>
                              <span className="kb-block md:kb-hidden">
                                Submit
                              </span>
                            </Button>
                            <Button
